...
panel %>%
  ...
  distinct_all() %>%
  mutate ( validate = validate_geo_code( .data$geo, nuts_year = 2016 )) %>%
  count( validate )
```
...
```{r save-to-tempdir}
saveRDS ( climate_data_recoded , file.path(tempdir(), "climate_panel_recoded.rds"), version = 2)
```

```{r copy-file, eval=TRUE}
# not evaluated
saveRDS( climate_data_recoded , file = file.path("data-raw", "climate-panel_recoded.rds"))
```
